...
let scaffold target_dir =
  ...
  let compiler_path = full_path "compiler" in
  if Sys.file_exists compiler_path then (
    Unix.chmod compiler_path 0o755;
    Printf.printf "[INFO] scaffolds - Set +x on compiler\n"
  );
